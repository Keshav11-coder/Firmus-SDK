...
public:
    ...
    {
        // Use model to compute required orientation based on desired acceleration

        auto T_vec = Matrix2::make_matrix<float, 3, 1>({{des_acc.x}, {des_acc.y}, {des_acc.z}}) *
                     _mass; // T = m * a
        ...
        Vector3 des_orientation(asin(T_hat(0, 0)), -asin(T_hat(1, 0)), yaw_reference);
